@echo off
REM ---------------------------------------------------------------------------
REM  ROV Imagery Processing (working title) - launcher
REM
...
set "ENV_ROOT=%LOCALAPPDATA%\CCR_ROV\rov_imagery_processing"
...
if exist "%VPY%" (
  "%VPY%" -c "import rov_imagery_processing.gui.app" >nul 2>&1
...
echo Installing ROV Imagery Processing...
...
REM ---- 4. run -------------------------------------------------------------
:run
"%VPYW%" -m rov_imagery_processing
if errorlevel 1 (
  echo.
  echo ROV Imagery Processing exited with an error. Running again with the
  echo console visible:
  echo.
  "%VPY%" -m rov_imagery_processing
  echo.
  pause
)
endlocal
