...
class ProcessPage(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        # height=1: an empty CTkFrame is 200px tall by default, and this row
        # is empty until the flight has GPR folders to offer.
        self.recent_row = ctk.CTkFrame(c1.body, fg_color="transparent", height=1)
        self.recent_row.grid(row=1, column=0, sticky="ew", pady=(8, 0))

        button(c1.body, "Check", self._check, "primary", width=140
               ).grid(row=2, column=0, sticky="w", pady=(10, 0))

        self.out = output_box(c1.body, wrap="word")
        self.out.grid(row=3, column=0, sticky="ew", pady=(10, 0))
        c1.add_grip(self.out, self.out.min_height)
    ...
    def _finish_setup(self, app, need_seed: bool) -> None:
        ...
        if outstanding:
            ...
            messagebox.showinfo(
                ...
                f"for this again.")
            self.app._log(f"Lightroom {app.version} set up for RAW develop")
        self._check()
