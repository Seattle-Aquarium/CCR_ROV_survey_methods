...
-- AHRS_ORIG_ALT : AHRS Origin Altitude (in meters above sea level)
--
-- How to use
...
--
-- CCR fix, 2026-09-18: the upstream version of this script (ArduPilot/ardupilot
-- master, libraries/AP_Scripting/applets/ahrs-set-origin.lua) checks
-- "AHRS_ORIG_ALT == 0" instead of "AHRS_ORIG_ALT:get() == 0" below. That
-- compares the Parameter object itself to a number, which Lua never treats as
-- equal, so the "no params set yet" branch never actually fires -- on a fresh
-- install the script proceeds straight to ahrs:set_origin() with whatever the
-- three parameters currently hold, including all-zero defaults if it happens
-- to run before the operator has typed the real coordinates in, and it never
-- retries after that (see the bare "return" once an origin is set). One line
-- fixed here; everything else is the published script unchanged.
...
function update()
    ...
    -- return if parameters have not been set (fixed: AHRS_ORIG_ALT:get(), not AHRS_ORIG_ALT)
    if AHRS_ORIG_LAT:get() == 0 and AHRS_ORIG_LON:get() == 0 and AHRS_ORIG_ALT:get() == 0 then
