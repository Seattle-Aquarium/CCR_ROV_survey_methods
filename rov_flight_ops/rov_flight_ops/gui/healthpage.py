...
from ..telemetry_cache import cache_dir_for, plan_windows
...
class HealthPage(ctk.CTkFrame):
    def __init__(self, master, app, scroll: bool = True):
        ...
        c1 = Card(body, "3.  Recordings",
                  "The logs folder of a flight. Each .mcap is checked for "
        ...
        c2 = Card(body, "4.  What the check found", "")
        ...
        c4 = Card(body, "5.  Autopilot logs (.BIN)",
    ...
    def _quick(self) -> None:
        ...
        # Downloads keep each type in its own folder -- logs/mcap beside
        # logs/BIN -- so the autopilot logs are looked for from logs/.
        bin_root = folder.parent if folder.name.lower() == "mcap" else folder
        self.bins = binlog.probe_bins(binlog.list_bins(bin_root))
        self._render()
        self._render_bins()
