...
Accepts either a .tlog file or a .csv file (tlog_to_csv.py-style output with
Date, Time, DVLx, DVLy columns), chosen automatically by file extension.
...
def optimize_path(
    ...
    if os.path.splitext(logfile)[1].lower() == '.csv':
        original_positions = meter_mark.process_csv(logfile)
    else:
...
def main():
    ...
    parser.add_argument(
        'logfile',
        help='Path to the .tlog file, or a .csv file from tlog_to_csv.py'
    )
    parser.add_argument(
